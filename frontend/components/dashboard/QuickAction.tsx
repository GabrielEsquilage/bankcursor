...
export function QuickAction({ label, icon, onClick }: QuickActionProps) {
  return (
    <button 
      onClick={onClick}
      className="group flex items-center gap-4 p-5 bg-black hover:bg-white/[0.03] transition-all duration-300"
    >
      ...
      <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/30 group-hover:text-blue-500 transition-colors">
        {label}
      </span>
    </button>
  );
